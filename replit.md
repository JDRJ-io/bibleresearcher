...
. This allows the app to show what each prophecy is about and jump to related verses.
* Load process: 
When the user toggles on the Prophecy columns, the app fetches prophecy_rows.txt (if not cached yet) and parses it into a Map of verseID to {P[], F[], V[]} as described. This is cached in memory. The UI will then show three small columns (often just indicators like counts or dots) labeled P | F | V for each verse currently in view. For any verse that has entries, the respective column might show a count or marker (e.g., if a verse has two Verifications, the V column might show "2" or a special icon). When the user clicks on a prophecy indicator (for example, clicking the "2" in the V column for 1Chr.10:13 in the above example), the app will fetch prophecy_index.json if it hasn’t already. This fetch is handled in a dedicated ProphecyWorker, offloading the potentially heavy JSON parse off the main thread. The worker looks up the specific prophecy ID (e.g., 128) and returns the full detail: summary and lists of verses. The UI then opens a side panel (drawer) showing the prophecy’s summary and the verses under each category (Prediction, Fulfillment, Verification), with each verse listed as a clickable link. From there, the user can click a verse reference to navigate to it (which will update the main view’s anchor to that verse). All fetched prophecy data is cached, so subsequent accesses are instant. Status: Like cross-references, the Prophecy system’s data layer is integrated and confirmed working via BibleDataAPI (the above flow is implemented), but the UI presentation is still being finalized. Currently, the Prophecy columns may appear but not yet display the actual counts/indicators without some additional UI work (this is actively being developed; the backend portion is done).
