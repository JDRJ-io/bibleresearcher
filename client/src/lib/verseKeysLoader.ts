/**
 * Verse Keys Loader - The Foundation
 * 
 * This file loads the verse keys first as the master row index.
 * All other data (translations, cross-refs, prophecy) plugs into this index.
 * 
 * From replit.md:
 * "verse-keys are the master row index; everything else—text, metadata, user content—simply plugs into that index."
 */
...
export async function loadVerseKeysCanonical(): Promise<string[]> {
  ...
  try {
    ...
    const response = await fetch('/verseKeys-canonical.txt');
    if (!response.ok) {
      throw new Error('Failed to load verse keys file');
    }
    ...
    console.error("Failed to load verse keys from file, using fallback:", error);
    
    // Fallback: Generate verse keys from Bible structure
    ...
    const books = [
      ...
      { name: "Exod", chapters: [22, 25, 22, 31, 23, 30, 25, 32, 35, 29, 10, 51, 22, 31, 27, 36, 16, 27, 25, 26, 36, 31, 33, 18, 40, 37, 21, 43, 46, 38, 18, 35, 23, 35, 35, 38, 29, 31, 43, 38] },
      ...
      { name: "Deut", chapters: [46, 37, 29, 49, 33, 25, 26, 20, 29, 22, 32, 32, 18, 29, 23, 22, 20, 22, 21, 20, 23, 30, 25, 22, 19, 19, 26, 68, 29, 20, 30, 52, 29, 12] },
      { name: "Josh", chapters: [18, 24, 17, 24, 15, 27, 26, 35, 27, 43, 23, 24, 33, 15, 63, 10, 18, 28, 51, 9, 45, 34, 16, 33] },
      { name: "Judg", chapters: [36, 23, 31, 24, 31, 40, 25, 35, 57, 18, 40, 15, 25, 20, 20, 31, 13, 31, 30, 48, 25] },
      { name: "Ruth", chapters: [22, 23, 18, 22] },
      { name: "1Sam", chapters: [28, 36, 21, 22, 12, 21, 17, 22, 27, 27, 15, 25, 23, 52, 35, 23, 58, 30, 24, 42, 15, 23, 29, 22, 44, 25, 12, 25, 11, 31, 13] },
      ...
      { name: "2Chr", chapters: [17, 18, 17, 22, 14, 42, 22, 18, 31, 19, 23, 16, 22, 15, 19, 14, 19, 34, 11, 37, 20, 12, 21, 27, 28, 23, 9, 27, 36, 27, 21, 33, 25, 33, 27, 23] },
      { name: "Ezra", chapters: [11, 70, 13, 24, 17, 22, 28, 36, 15, 44] },
      { name: "Neh", chapters: [11, 20, 32, 23, 19, 19, 73, 18, 38, 39, 36, 47, 31] },
      { name: "Esth", chapters: [22, 23, 15, 17, 14, 14, 10, 17, 32, 3] },
      ...
      { name: "Ps", chapters: [6, 12, 8, 8, 12, 10, 17, 9, 20, 18, 7, 8, 6, 7, 5, 11, 15, 50, 14, 9, 13, 31, 6, 10, 22, 12, 14, 9, 11, 12, 24, 11, 22, 22, 28, 12, 40, 22, 13, 17, 13, 11, 5, 26, 17, 11, 9, 14, 20, 23, 19, 9, 6, 7, 23, 13, 11, 11, 17, 12, 8, 12, 11, 10, 13, 20, 7, 35, 36, 5, 24, 20, 28, 23, 10, 12, 20, 72, 13, 19, 16, 8, 18, 12, 13, 17, 7, 18, 52, 17, 16, 15, 5, 23, 11, 13, 12, 9, 9, 5, 8, 28, 22, 35, 45, 48, 43, 13, 31, 7, 10, 10, 9, 8, 18, 19, 2, 29, 176, 7, 8, 9, 4, 8, 5, 6, 5, 6, 8, 8, 3, 18, 3, 3, 21, 26, 9, 8, 24, 13, 10, 7, 12, 15, 21, 10, 20, 14, 9, 6] },
      { name: "Prov", chapters: [33, 22, 35, 27, 23, 35, 27, 36, 18, 32, 31, 28, 25, 35, 33, 33, 28, 24, 29, 30, 31, 29, 35, 34, 28, 28, 27, 28, 27, 33, 31] },
      { name: "Eccl", chapters: [18, 26, 22, 16, 20, 12, 29, 17, 18, 20, 10, 14] },
      { name: "Song", chapters: [17, 17, 11, 16, 16, 13, 13, 14] },
      { name: "Isa", chapters: [31, 22, 26, 6, 30, 13, 25, 22, 21, 34, 16, 6, 22, 32, 9, 14, 14, 7, 25, 6, 17, 25, 18, 23, 12, 21, 13, 29, 24, 33, 9, 20, 24, 17, 10, 22, 38, 22, 8, 31, 29, 25, 28, 28, 25, 13, 15, 22, 26, 11, 23, 15, 12, 17, 13, 12, 21, 14, 21, 22, 11, 12, 19, 12, 25, 24] },
      ...
      { name: "Ezek", chapters: [28, 10, 27, 17, 17, 14, 27, 18, 11, 22, 25, 28, 23, 23, 8, 63, 24, 32, 14, 49, 32, 31, 49, 27, 17, 21, 36, 26, 21, 26, 18, 32, 33, 31, 15, 38, 28, 23, 29, 49, 26, 20, 27, 31, 25, 24, 23, 35] },
      ...
      { name: "Obad", chapters: [21] },
      { name: "Jonah", chapters: [17, 10, 10, 11] },
      { name: "Mic", chapters: [16, 13, 12, 13, 15, 16, 20] },
      { name: "Nah", chapters: [15, 13, 19] },
      { name: "Hab", chapters: [17, 20, 19] },
      { name: "Zeph", chapters: [18, 15, 20] },
      { name: "Hag", chapters: [15, 23] },
      { name: "Zech", chapters: [21, 13, 10, 14, 11, 15, 14, 23, 17, 12, 17, 14, 9, 21] },
      { name: "Mal", chapters: [14, 17, 18, 6] },
      { name: "Matt", chapters: [25, 23, 17, 25, 48, 34, 29, 34, 38, 42, 30, 50, 58, 36, 39, 28, 27, 35, 30, 34, 46, 46, 39, 51, 46, 75, 66, 20] },
      ...
      { name: "Phil", chapters: [30, 30, 21, 23] },
      { name: "Col", chapters: [29, 23, 25, 18] },
      { name: "1Thess", chapters: [10, 20, 13, 18, 28] },
      { name: "2Thess", chapters: [12, 17, 18] },
