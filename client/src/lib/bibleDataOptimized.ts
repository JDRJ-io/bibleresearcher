import { loadTranslationSecure, loadCrossReferences, loadContextGroups } from './supabaseLoader';
...
export async function populateTranslation(
  ...
  try {
    console.log(`Populating ${translationId} for verses ${startIndex}-${endIndex || verses.length}`);
    ...
    for (let i = startIndex; i < end && i < verses.length; i++) {
      ...
      if (!verse.text[translationId]) {
        verse.text[translationId] = `[${verse.reference} - ${translationId} not available]`;
      }
    }
...
export async function populateContextGroups(verses: BibleVerse[]): Promise<void> {
  try {
...
export function findVerseIndex(reference: string): number {
  ...
  return globalVerseStructure.findIndex(verse => 
