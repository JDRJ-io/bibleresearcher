...
self.addEventListener('message', async (event: MessageEvent<ProphecyMessage>) => {
  ...
  if (type === 'PARSE_PROPHECY_FILES') {
    try {
      ...
      console.log(`✅ ProphecyWorker: Parsed ${Object.keys(verseRoles).length} verses with prophecy roles and ${Object.keys(prophecyIndexData).length} prophecy definitions`);
