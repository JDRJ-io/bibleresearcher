...
export function ExpandedVerseOverlay({ verse, onClose, onStrongsClick }: ExpandedVerseOverlayProps) {
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="flex items-center justify-between border-b pb-4">
            <div>
              ...
              <p className="text-sm opacity-75 mt-1">King James Version</p>
            </div>
            <Button
              variant="ghost"
              size="sm"
          ...
          {/* Expanded Verse Text */}
          <div className="text-lg leading-relaxed" style={{ color: 'var(--text-color)' }}>
            {verse.text.KJV}
          </div>
          ...
          {verse.strongsWords && verse.strongsWords.length > 0 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold flex items-center">
                <Languages className="w-5 h-5 mr-2" style={{ color: 'var(--accent-color)' }} />
                  ...
                  {verse.strongsWords.map((word, index) => (
                    ...
                    >
                      <div className="text-center space-y-1">
                        <div className="font-hebrew text-lg">{word.original}</div>
          ...
          {verse.crossReferences && verse.crossReferences.length > 0 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold flex items-center">
                <Link className="w-5 h-5 mr-2" style={{ color: 'var(--accent-color)' }} />
                Cross References
              </h3>
              <div className="space-y-3">
                {verse.crossReferences.map((ref, index) => (
                  ...
                  >
                    <div className="font-medium text-sm mb-1" style={{ color: 'var(--accent-color)' }}>
                      {ref.reference}
                    </div>
